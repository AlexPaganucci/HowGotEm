...
public interface SizeRepository extends JpaRepository<Size, Long>{
	...
	@Modifying
	@Query(
			value = "UPDATE Size s SET s.quantityAvailable = s.quantityAvailable - :quantity WHERE s.shoe.id = :shoeId AND s.id = :sizeId"
	)
	void decreaseQuantity(@Param("shoeId") Long shoeId, @Param("sizeId") Long sizeId, @Param("quantity") int quantity);	
	
    @Query(
    		nativeQuery = true,
    		value = "SELECT * FROM SIZES s WHERE s.SHOE_ID = :shoeId AND s.ID = :sizeId"
    		)
